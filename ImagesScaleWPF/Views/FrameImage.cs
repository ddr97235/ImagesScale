    ...
    public partial class FrameImage : Freezable
    {
        ...
        public static readonly DependencyProperty SourceProperty =
            DependencyProperty.Register(nameof(Source),
                                        ...
                                        {
                                            PropertyChangedCallback = (d, _) =>
                                            {
                                                FrameImage fi = (FrameImage)d;
                                                fi.InvalidateProperty(FrameRectProperty);
                                                fi.InvalidateProperty(FrameProperty);
                                            }
                                        });
        /// <summary>Фрагмент кадра <see cref="SolidColorBrush"/>.</summary>
            ...
            DependencyProperty.RegisterReadOnly(nameof(Frame),
                                                ...
                                                new PropertyMetadata(null)
                                                {
                                                    CoerceValueCallback = (d, value) =>
                                                    {
                                                        ...
                                                        Int32Rect intRect = frame.FrameRect;
                                                        CroppedBitmap? result;

                                                        if (frame.Source is BitmapSource source && !intRect.IsEmpty)
                                                        {
                                                            // Rect rect = frame.FrameRectRelative;
                                                            ...
                                                            result = new CroppedBitmap(source, intRect);
                                                        }
                                                        else
                                                        {
                                                            result = null;
        ...
        /// <summary>Прямоугольник фрагмента кадра в абсолютных единицах.</summary>
        ...
        /// <summary>Прямоугольник фрагмента кадра в относительных единицах.</summary>
        public Rect FrameRectRelative
        {
            get => (Rect)GetValue(FrameRectRelativeProperty);
            set => SetValue(FrameRectRelativeProperty, value);
        }

        // Using a DependencyProperty as the backing store for ViewBox.  This enables animation, styling, binding, etc...
        private static readonly DependencyProperty FrameRectRelativeProperty =
            DependencyProperty.Register(nameof(FrameRectRelative),
                                                typeof(Rect),
                                                typeof(FrameImage),
                                                new PropertyMetadata(new Rect())
                                                {
                                                    PropertyChangedCallback = (d, _) => ((FrameImage)d).InvalidateProperty(FrameRectProperty),
                                                });
